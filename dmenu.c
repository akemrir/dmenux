...
int main(int argc, char *argv[]) {
    ...
    for(i = 1; i < argc; i++) {
        ...
        if(!strcmp(argv[i], "-v")) {  /* prints version information */
            ...
            topbar = False;
        } else if(!strcmp(argv[i], "-q")) {  /* not show any items if the search string is empty */
            quiet = True;
        } else if(!strcmp(argv[i], "-f")) {  /* grabs keyboard before reading stdin */
            fast = True;
        } else if(!strcmp(argv[i], "-z")) {  /* enable fuzzy matching */
            match = matchfuzzy;
        } else if(!strcmp(argv[i], "-r")) { /* filter all items currently shown in the list */
            filter = True;
            ...
            maskin = True;
        } else if(!strcmp(argv[i], "-noinput")) { /* ignores input from stdin */
            noinput = True;
        } else if(!strcmp(argv[i], "-t")) { /* dmenu uses fuzzy matching */
            match = matchtok;
        } else if(i+1 == argc) {
            usage();
        /* these options take one argument */
            ...
            yoffset = atoi(argv[++i]);
        } else if(!strcmp(argv[i], "-w")) {  /* minimum width menu window */
            width = atoi(argv[++i]);
...
void cleanup(void) {
    ...
    if(dimopacity > 0) XDestroyWindow(dc->dpy, dim);
    XUngrabKeyboard(dc->dpy, CurrentTime);
    XUngrabPointer(dc->dpy, CurrentTime);
    freedc(dc);
}

const char *createmaskinput(char *maskinput, int length) {
   if (length <= 0) {
      *maskinput = '\0';
   } else {
...
char *strchri(const char *s, int c) {
    ...
    if(isupper(c)) {
        return strchr(s, tolower(c));
    } else {
        return strchr(s, toupper(c));
    }
}

void pointermove(XEvent *e) {
    Item *item;
...
void matchstr(void) {
    ...
    for(item = items; item && item->text; item++) {
        ...
        if(i != tokc) continue; /* not all tokens match */
...
void run(void) {
    ...
    while(running && !XNextEvent(dc->dpy, &ev)) {
        ...
        switch(ev.type) {
        case MotionNotify:
            while(XCheckTypedEvent(dc->dpy, MotionNotify, &ev)) (void)0;
            pointermove(&ev);
            break;
        case ButtonPress:
            buttonpress(&ev);
            break;
        case Expose:
            if(ev.xexpose.count == 0)  mapdc(dc, win, mw, mh);
            break;
        case KeyPress:
            keypress(&ev.xkey);
            break;
        ...
        case VisibilityNotify:
            if(ev.xvisibility.state != VisibilityUnobscured) XRaiseWindow(dc->dpy, win);
            break;
        }
    }
}
